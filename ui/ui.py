...
def call_determenant(call):
    ...
    if session.select.stage == Stage.endYearSelect.value:
        if call.data == "left" or call.data == "right":
            markup_date_rechanger(session,call.data)
        elif call.data.find("set") != -1:
            asyncio.run(functional_step_selecter(session,int(call.data.split("_")[1])))
    if session.select.stage == Stage.startYearSelect.value:
        if call.data == "left" or call.data == "right":
            markup_date_rechanger(session,call.data)
        elif call.data.find("set") != -1:
            asyncio.run(functional_end_year_selecter(session,int(call.data.split("_")[1])))

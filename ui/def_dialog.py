...
async def cron_selecter(session):
    ...
    session.message_markup.first_date = 1920
...
async def functional_step_selecter(session,num):
    if session.select.start_date <= num:
        session.select.end_date = num
        session.select.stage = Stage.stepSelect.value
        session.messages_id.append(session.bot.send_message(session.id,stroke_pointer(f"Был указан конечный год: {num}")).id)
        await asyncio.sleep(1)
        markup_clearer(session)
        stage_messages_id_clearer(session)
        session.message_markup = Markup()
...
async def functional_treshold_selecter(session,message):
    ...
    return None
async def functional_end_year_selecter(session,num):
    stage_messages_id_clearer(session)
    markup_clearer(session)
    session.select.stage = Stage.endYearSelect.value
    session.select.start_date = num
    session.messages_id.append(session.bot.send_message(session.id,stroke_pointer(f"Был выбран начальный год: {num};")).id)
    await asyncio.sleep(1)
    session.message_markup = Markup()
    session.message_markup.markup = markup_date_geter(1920)
    session.message_markup.id = session.bot.send_message(session.id,stroke_pointer("Укажите конечный год:"),reply_markup=session.message_markup.markup).id
